...
public partial class MoveGizmo : Node, IGizmo
{
	...
	private const float GizmoArrowGap = 0.7f;
	...
	private void CreateSurfTool()
	{
		for (int i = 0; i < 3; i++)
		{
			...
			foreach (float dirSign in new float[] { 1f, -1f })
			{
				Vector3 dvec = _ivec * dirSign;
				Vector3[] arrow = [
					_nivec * 0f + dvec * GizmoArrowGap,
					_nivec * 0.01f + dvec * GizmoArrowGap,
					_nivec * 0.01f + dvec * GizmoArrowOffset,
					_nivec * 0.065f + dvec * GizmoArrowOffset,
					_nivec * 0f + dvec * (GizmoArrowOffset + Gizmos.GizmoArrowSize),
				];
				...
				{
					Basis ma = new(dvec, k * arrowSidesStep);
					Basis mb = new(dvec, (k + 1) * arrowSidesStep);
	...
	private Transform3D GetMovePivot()
	{
		Transform3D pivot = Gizmos.GetCenterPivot([.. Targets]);
		if (CreatorService.Interface.GizmoLocalSpace && Targets.Count == 1)
		{
			pivot.Basis = Targets[0].GetGlobalTransform().Basis.Orthonormalized();
		}
		return pivot;
	}
	...
	private void RedrawGizmo()
	{
		...
		Transform3D pform = GetMovePivot();
	...
	{
		Transform3D pivot = GetMovePivot();
		...
		int colAxis = -1;
			...
			for (int i = 0; i < 3; i++)
			{
				foreach (float dirSign in new float[] { 1f, -1f })
				{
					Vector3 grabberPos = pivot.Origin + pivot.Basis.GetColumn(i).Normalized() * dirSign * gizmoScale * (GizmoArrowOffset + Gizmos.GizmoArrowSize * 0.5f);
			...
			HighlightAxis(colAxis, colDir);
	}

	private void HighlightAxis(int axis, int dir)
	{
		for (int i = 0; i < 3; i++)
		{
			_moveGizmo[i].SurfaceSetMaterial(0, i == axis && dir == 1 ? _gizmoHoverColor[i] : _gizmoColor[i]);
			_moveGizmo[i].SurfaceSetMaterial(1, i == axis && dir == -1 ? _gizmoHoverColor[i] : _gizmoColor[i]);
		}
	...
	private void DragTransform(Vector3 rayOrigin, Vector3 rayNormal, Vector3 cameraNormal)
	{
		...
		float dist = motionMask.Dot(intersection.Value - click.Value);
		dist = Mathf.Snapped(dist, CreatorService.Interface.MoveSnapping);
		Vector3 motion = dist * motionMask;
