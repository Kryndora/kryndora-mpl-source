...
public partial class ExplorerItemContextMenu : ContextMenu
{
	...
	public override void _Ready()
	{
		...
		if (isSingle)
		{
			...
			AddIconItem("folder", "Add Folder", 3);
			...
			case 3: // Add folder
				{
					if (Target != null)
					{
						if (Target.Parent != null && !Target.GetType().IsDefined(typeof(StaticAttribute), true))
						{
							context.History.GroupInstances(targets, CreatorHistory.GroupAsEnum.Folder);
						}
						else
						{
							Folder folder = targets[0].Root.New<Folder>();
							folder.Name = "Folder";
							folder.CreatorInserted();
							context.History.CreateInstances([folder], Target);
							context.Selections.SelectOnly(folder);
						}
					}
					break;
				}
