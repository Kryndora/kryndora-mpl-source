...
public partial class ExplorerTree : Tree
{
	...
	public override void _DropData(Vector2 atPosition, Variant data)
	{
		...
		List<(Instance instance, Instance? oldParent, int oldIndex, Vector3? oldPos)> originalState = [];
		List<(Instance instance, Instance? newParent, int newIndex, bool snap)> finalState = [];
		...
		foreach (Instance draggedInstance in sortedDraggedInstances)
		{
			if (parentTo != null)
			{
				...
				try
				{
					...
					Vector3? oldPos = draggedInstance is Dynamic dynOld ? dynOld.Position : null;
					originalState.Add((draggedInstance, oldParent, oldIndex, oldPos));
					...
					bool snap = parentTo is Tool && draggedInstance is Dynamic && oldParent != parentTo;
					finalState.Add((draggedInstance, parentTo, adjustedIndex, snap));
				}
				catch (Exception ex)
				{
					PT.PrintErr(ex);
		...
		if (originalState.Count > 0)
		{
			...
			{
				Root.CreatorContext.Selections.DeselectAll();
				foreach (var (instance, newParent, newIndex, snap) in finalState)
				{
					if (newParent != null)
					{
						...
						if (snap && newParent is Tool toolParent && instance is Dynamic dynChild)
						{
							dynChild.Position = toolParent.Position;
						}
			...
			history.AddUndoCallback(new((_) =>
			{
				...
				{
					var (instance, oldParent, oldIndex, oldPos) = originalState[i];
					if (oldParent != null)
					{
						instance.Parent = oldParent;
						oldParent.MoveChild(instance, oldIndex);
						if (oldPos.HasValue && instance is Dynamic dynChild)
						{
							dynChild.Position = oldPos.Value;
						}
						Root.CreatorContext.Selections.Select(instance);
					}
				}
