...
public static class ProjectManager
{
	...
	private const string GitIgnoreContent = "# Kryndora specific ignores\n.poly/\n";
	...
	public static async Task<RecentData[]> GetRecents(bool loadData = true)
	{
		...
		if (File.Exists(recentsPath))
		{
			...
			RecentData?[] loadedData = await Task.WhenAll(tasks);
			foreach (RecentData? loadedRecent in loadedData)
			{
				if (loadedRecent.HasValue)
				{
					finalData.Add(loadedRecent.Value);
				}
			}
