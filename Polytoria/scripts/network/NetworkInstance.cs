...
public class NetworkInstance
{
	...
	public void CreateServer(int port = DefaultPort, int maxChannels = 3, string bindAddress = "*")
	{
		if (string.IsNullOrWhiteSpace(bindAddress))
			bindAddress = "*";

		Error e = _peer.CreateHostBound(bindAddress, port, DefaultCapacity, maxChannels);
	...
	private void DrainEvents()
	{
		try
		{
			while (_mainThreadEventQueue.TryDequeue(out DeferredNetworkEvent? e))
			{
				...
				{
					GD.PushError("Error dispatching network event: ", ex);
				}
			}
		}
		finally
