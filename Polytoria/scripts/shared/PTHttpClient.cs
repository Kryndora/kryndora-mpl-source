...
public partial class PTHttpClient
{
	...
	public PTHttpClient()
	{
		DefaultRequestHeaders["User-Agent"] = $"{Globals.AppDisplayName} Client {Globals.AppVersion}";
	}
		...
		Callable.From(() =>
		{
			...
			async void a()
			{
				...
				try
				{
					...
					req = new() { DownloadChunkSize = DefaultDownloadChunkSize };
					...
					req.RequestCompleted += (result, responseCode, responseHeaders, responseBody) =>
					{
						try
						{
							if (result != (long)HttpRequest.Result.Success || responseCode < 100)
							{
								tcs.TrySetException(new HttpRequestException($"HttpRequest failed with result {result}, response code {responseCode}."));
								return;
							}
