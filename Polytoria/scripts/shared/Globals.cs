...
{
	public const string AppDisplayName = "Kryndora";
	...
	private static string ResolvePlatformEndpoint()
	{
		try
		{
			...
			string cfg = System.IO.Path.Combine(exeDir, "kryndora_endpoint.txt");
	...
	public const string LocalDevToken = "crystalforge-local-dev";
	...
	public override void _EnterTree()
	{
		...
		PT.Print($"{AppDisplayName} v{AppVersion}");
		PT.Print(MainEndpoint);
	...
	public static NetworkedObject? LoadNetworkedObject(string className, World? root = null)
	{
		...
		if (type != null)
		{
			...
			{
				throw new InvalidOperationException($"Failed to create datamodel instance '{className}': {ex.InnerException.Message}", ex.InnerException);
			}
	...
	private static Texture2D LoadCachedTexture(Dictionary<string, Texture2D> cache, string key, string directoryPath, string fallbackKey)
	{
		...
		// ResourceLoader returns null when the .import/.ctex cache is missing (e.g. freshly added PNG).
		// Fall back to direct Image loading so unimported PNGs still display correctly.
		if (texture == null && path.EndsWith(".png"))
		{
		...
		if (texture == null)
		{
			// Still couldn't load — fall back to the fallback icon, not a crash.
			if (key == fallbackKey)
				throw new InvalidOperationException($"Failed to load texture at '{path}'.");

			PT.PrintWarn($"[Icons] Could not load texture '{path}', falling back to '{fallbackKey}'.");
			Texture2D fallbackTexture = LoadCachedTexture(cache, fallbackKey, directoryPath, fallbackKey);
			cache[key] = fallbackTexture;
			return fallbackTexture;
		}
	...
	private static string? ResolveTexturePath(string directoryPath, string name)
	{
		...
		// Folder has a custom bitmap icon, so prefer it over the built-in vector fallback.
		if (name == "Folder" && (ResourceLoader.Exists(pngPath) || Godot.FileAccess.FileExists(pngPath)))
		{
			return pngPath;
		}
		...
		// ResourceLoader.Exists requires a compiled .import cache.
		// Also check raw file existence so freshly added PNGs are found before Godot imports them.
		if (ResourceLoader.Exists(pngPath) || Godot.FileAccess.FileExists(pngPath))
		{
			return pngPath;
		}
	...
	public static Dictionary<string, string> ReadCmdArgs()
	{
		...
		for (int i = 0; i < args.Length; i++)
		{
			...
			if (TryReadKryndoraLaunchUrl(result, arg))
			{
				continue;
			}
	...
	private static bool TryReadKryndoraLaunchUrl(Dictionary<string, string> result, string value)
	{
		if (!value.StartsWith("kryndora:", StringComparison.OrdinalIgnoreCase))
			return false;
