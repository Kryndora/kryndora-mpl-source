...
public partial class RendererViewport : SubViewport
{
	...
	public async Task AddAvatar(int id, AvatarPhotoTypeEnum photoType = AvatarPhotoTypeEnum.FullAvatar, string? bodyOverride = null, string? shirtOverride = null)
	{
		...
		{
			ptm.ApplyBodyStyle(bodyOverride);
			await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
			await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
		}

		if (shirtOverride != null)
		{
			ptm.SetTestShirtStyle(shirtOverride == "kryndora" ? "kryndora" : null);
			await ToSignal(GetTree(), SceneTree.SignalName.ProcessFrame);
		}
	...
	public async Task<byte[]> SavePng()
	{
		...
		// A virtual X display does not reliably emit FramePostDraw on its own.
		// Advance and force a few frames before reading the viewport texture.
