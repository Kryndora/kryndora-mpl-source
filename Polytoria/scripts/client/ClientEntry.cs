...
public sealed partial class ClientEntry : Node3D
{
	...
	public int LocalTestBaseUserID = 1;
	...
	public async void Entry(ClientEntryData? data = null)
	{
		...
		catch (Exception ex)
		{
			...
			{
				OS.Alert("Could not join this local game. Please make sure Kryndora Launcher is still open and try again.", "Join failed");
	...
	private async Task EntryAsync(ClientEntryData? data = null)
	{
...
#if ALLOW_SELFHOST
		bool hasExplicitConnectTarget = cmdargs.ContainsKey("address") && cmdargs.ContainsKey("port");
		...
		cmdargs.TryGetValue("bind", out string? bindAddress);
		...
		cmdargs.TryGetValue("creator-user-id", out string? creatorUserIDStr);

		connectAddress ??= OS.HasFeature("macos") ? "192.168.2.74" : "127.0.0.1";
		portStr ??= OS.HasFeature("macos") ? "24250" : "24221";
		nPlrStr ??= "1";
		...
		if (creatorUserIDStr != null)
		{
			LocalTestBaseUserID = int.Parse(creatorUserIDStr);
		}
#endif
		networkMode ??= "client";
		...
		if (isServer)
		{
			if (token != null)
			{
				...
				try
				{
					...
					PT.Print($"{Globals.AppDisplayName} Server Info ----");
					...
					networkService.CreateServer(fport, bindAddress ?? "*");
				}
				catch (Exception ex)
				{
					// Error bruh
			...
			else
			{
				...
				try
				{
					...
					networkService.CreateServer(port, bindAddress ?? "*");
					if (DebugAgent != null)
		...
		if (isClient)
		{
			AddChild(new Voice.VoiceService { Name = "VoiceService" }, true);

			if (token != null && !hasExplicitConnectTarget)
			{
				PT.Print($"Connecting to {Globals.AppDisplayName}...");
				...
				try
				{
					...
					PT.Print($"{Globals.AppDisplayName} Network Info ----");
	...
	{
		TestClientCount++;
		int plrID = LocalTestBaseUserID > 0 ? LocalTestBaseUserID + TestClientCount - 1 : TestClientCount;
