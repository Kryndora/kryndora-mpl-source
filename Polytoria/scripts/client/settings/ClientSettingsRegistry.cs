...
public static class ClientSettingsRegistry
{
	...
	private static Dictionary<string, SettingDef> Build()
	{
		...
		defs.Add(ClientSettingKeys.General.PushToTalk,
			new SettingDef<bool>
			{
				Key = ClientSettingKeys.General.PushToTalk,
				SectionKey = "general",
				Label = "Voice: Push-to-talk",
				Description = "When on, hold V to talk in voice chat. When off, your mic is open (voice-activated) while unmuted.",
				ValueKind = SettingValueKind.Bool,
				ControlKind = SettingControlKind.Toggle,
				DefaultValue = false
			});
...
defs.Add(ClientSettingKeys.Display.UiScale,
		...
		defs.Add(ClientSettingKeys.Overlay.ShowFpsAndPing,
			new SettingDef<bool>
			{
				Key = ClientSettingKeys.Overlay.ShowFpsAndPing,
				SectionKey = "overlay",
				Label = "Show FPS And Ping",
				Description = "Display FPS and ping in the top right corner.",
				ValueKind = SettingValueKind.Bool,
				ControlKind = SettingControlKind.Toggle,
				DefaultValue = false
			});
