...
{
	public const string PolytoriaFolderName = "Kryndora/";
	...
	public static void OpenScript(Script script)
	{
		...
		if (script.LinkedScript != null)
		{
			...
			if (scriptPath == null)
			{
				if (TryRecoverScriptLink(script, out scriptPath))
				{
					PT.Print("Recovered script link: ", script.Name, " -> ", scriptPath);
					OpenFile(scriptPath);
					return;
				}

				Interface.PopupAlert("Script's file reference is invalid, and Kryndora could not find a matching script file in this project.");
				return;
			}
		...
		{
			if (TryRecoverScriptLink(script, out string? scriptPath))
			{
				PT.Print("Recovered script link: ", script.Name, " -> ", scriptPath);
				OpenFile(scriptPath);
				return;
			...
			if (TryCreateMissingScriptFile(script, out scriptPath))
			{
				PT.Print("Created missing script file: ", script.Name, " -> ", scriptPath);
				OpenFile(scriptPath);
				return;
			}

			Interface.PopupAlert("Script does not have a file reference, and Kryndora could not find a matching script file in this project.");
		}
	}

	private static bool TryCreateMissingScriptFile(Script script, out string scriptPath)
	{
		scriptPath = "";
		if (CurrentSession == null || script.Root == null) return false;

		string folder = script switch
		{
			ClientScript => "scripts/client",
			ModuleScript => "scripts/modules",
			_ => "scripts/server"
		};

		string fileName = script.CreateLuaFileName();
		string safeFileName = string.IsNullOrWhiteSpace(fileName) ? $"{script.ClassName}.luau" : fileName;
		scriptPath = $"{folder}/{safeFileName}".SanitizePath();
		int duplicate = 2;
		while (CurrentSession.FileExists(scriptPath))
		{
			string baseName = safeFileName.GetBaseName();
			string extension = safeFileName.GetExtension();
			scriptPath = $"{folder}/{baseName}{duplicate}.{extension}".SanitizePath();
			duplicate++;
		}

		string absolutePath = CurrentSession.GlobalizePath(scriptPath);
		string? baseFolder = Path.GetDirectoryName(absolutePath);
		if (!string.IsNullOrWhiteSpace(baseFolder) && !Directory.Exists(baseFolder))
		{
			Directory.CreateDirectory(baseFolder);
		}

		File.WriteAllText(absolutePath, script.Source);
		script.LinkedScript = script.Root.Assets.GetFileLinkByPath(scriptPath);
		CurrentSession.RescanFolder();
		CurrentSession.SaveFileIndex();
		Interface.StatusBar?.SetStatus($"Created script file: {scriptPath}");
		return true;
	}

	private static bool TryRecoverScriptLink(Script script, out string recoveredPath)
	{
		recoveredPath = "";
		if (CurrentSession == null || script.Root == null) return false;
		...
		ScriptTypeEnum expectedType = script switch
		{
			ServerScript => ScriptTypeEnum.Server,
			ClientScript => ScriptTypeEnum.Client,
			ModuleScript => ScriptTypeEnum.Module,
			_ => ScriptTypeEnum.Unknown
		};

		string projectFolder = CurrentSession.ProjectFolderPath;
		if (!Directory.Exists(projectFolder)) return false;

		string expectedFileName = script.CreateLuaFileName();
		string normalizedScriptName = NormalizeScriptCandidateName(script.Name);

		var candidates = Directory.EnumerateFiles(projectFolder, "*.*", SearchOption.AllDirectories)
			.Where(path => Globals.ScriptFileExtensions.Contains(path.GetExtension()))
			.Where(path => !path.StartsWith(CurrentSession.PolyFolderPath, StringComparison.OrdinalIgnoreCase))
			.Select(path =>
			{
				string relativePath = Path.GetRelativePath(projectFolder, path).SanitizePath();
				string scriptName = GetScriptNameFromPath(relativePath);
				ScriptTypeEnum scriptType = GetScriptTypeFromPath(relativePath);
				int score = 0;

				if (relativePath.GetFile().Equals(expectedFileName, StringComparison.OrdinalIgnoreCase))
				{
					score += 100;
				}
				...
				if (NormalizeScriptCandidateName(scriptName) == normalizedScriptName)
				{
					score += 35;
				}
				...
				if (expectedType == ScriptTypeEnum.Server && relativePath.StartsWith("scripts/server/", StringComparison.OrdinalIgnoreCase))
				{
					score += 10;
				}
				else if (expectedType == ScriptTypeEnum.Client && relativePath.StartsWith("scripts/client/", StringComparison.OrdinalIgnoreCase))
				{
					score += 10;
				}

				return new { RelativePath = relativePath, Score = score };
			})
		...
		recoveredPath = candidates[0].RelativePath;
		script.LinkedScript = script.Root.Assets.GetFileLinkByPath(recoveredPath);
		CurrentSession.SaveFileIndex();
		Interface.StatusBar?.SetStatus($"Recovered script link: {script.Name} -> {recoveredPath}");
		return true;
	}

	private static string NormalizeScriptCandidateName(string value)
	{
	...
	private void StartLocalTestServer(string placeFilePath, string entryPath, string debugID, int port, bool isSubplace = false, Vector3? spawnPos = null)
	{
		...
		if (PolyCreatorAPI.UserID > 0)
		{
			args.InsertRange(0, ["-creator-user-id", PolyCreatorAPI.UserID.ToString()]);
		}
