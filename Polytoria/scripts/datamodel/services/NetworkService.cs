...
public sealed partial class NetworkService : Instance
{
	...
	private const int EmptyHeartbeatsBeforeShutdown = 2;
	...
	public const string TerminationMessage = "Your Kryndora account has been terminated, please visit the website for more info.";
	...
	private string? _clientEndpointLabel;

	public event Action? ServerStarted;
	/// <summary>Raised on the client when proximity voice data is received from another player. (speakerUserId, opus)</summary>
	public event Action<int, byte[]>? VoiceReceived;
	/// <summary>Max distance (studs) at which proximity voice is heard.</summary>
	public const float VoiceRange = 45f;
	...
	private int _emptyHeartbeats = 0;
	...
	public void CreateServer(int port = 24221, string bindAddress = "*")
	{
		SetupNetwork();
		InitNodes();
		Root.ListenToNetworkService();
		...
		NetInstance.CreateServer(port, bindAddress: bindAddress);
	...
	public async void CreateClient(string address, int port = 24221)
	{
		try
		{
			_clientEndpointLabel = $"{address}:{port}";
			SetupNetwork();
		...
		{
			PT.PrintErr("Client network start failed: ", ex);
			DisconnectSelf("Could not connect to the local server.", DisconnectionCodeEnum.ConnectionFailure);
		}
	}
	...
	private async void ServerSendHeartbeat()
	{
		...
		{
			PT.PrintErr("Heartbeat failed: ", ex);
		}

		if (_heartbeatCount > HeartbeatBeforeCheckPlayers)
		{
			bool noPlayers = World.Current!.Players.AbsolutePlayersCount <= 0;
			bool noPeers = ActivePeerIDs.Count(id => id != 1) <= 0;
			if (noPlayers || noPeers)
			{
				_emptyHeartbeats++;
				PT.Print($"Server empty ({_emptyHeartbeats}/{EmptyHeartbeatsBeforeShutdown})");
				if (_emptyHeartbeats >= EmptyHeartbeatsBeforeShutdown)
				{
	...
	private void OnPlayerAdded(Player player)
	{
		...
		if (IsServer)
		{
			_ = ReportPresenceJoin(player.UserID);
		}
		OnPlayerChanged();
	}
	...
	private void OnPlayerRemoved(Player player)
	{
		...
		if (IsServer)
		{
			_ = ReportPresenceLeft(player.UserID);
		}
		OnPlayerChanged();
	}

	private static readonly PTHttpClient _presenceClient = new();

	private static async Task ReportPresenceLeft(int userId)
	{
		try
		{
			string endpoint = Globals.MainEndpoint;
			if (System.Environment.GetEnvironmentVariable("KRYNDORA_DEDICATED") == "1" && endpoint.Contains("kryndora.lol", StringComparison.OrdinalIgnoreCase))
				endpoint = "http://127.0.0.1:4000/";

			using HttpResponseMessage response = await _presenceClient.PostAsync(
				endpoint.TrimEnd('/') + "/api/local/presence/leave?userId=" + userId,
	...
	private static async Task ReportPresenceJoin(int userId)
	{
		try
		{
			Dictionary<string, string> args = Globals.ReadCmdArgs();
			args.TryGetValue("platformgameid", out string? gameId);
			args.TryGetValue("port", out string? port);
			...
			using HttpResponseMessage response = await _presenceClient.PostAsync(
				endpoint.TrimEnd('/') + "/api/local/presence/join?userId=" + userId + "&gameId=" + (gameId ?? "0") + "&port=" + (port ?? "0"),
				new ByteArrayContent([]));
		}
		catch
		{
	...
	private async void NetAuthResponse(int testUserID, string userToken, int networkMode, int platform, string platformStr, byte[] pk)
	{
		...
		try
		{
			...
			else
			{
				...
				APIMeResponse me = await PolyAPI.GetCurrentUser(userToken);
				validateRes = new() { CanChat = true, UserID = me.Id, IsCreator = false, IsAgeRestricted = false };
				userData = await PolyAPI.GetUserFromID(validateRes.UserID);
			}

			if (!validateRes.IsCreator)
			{
				Globals.ReadCmdArgs().TryGetValue("owneruserid", out string? ownerStr);
				if (int.TryParse(ownerStr, out int ownerUserId) && ownerUserId > 0 && userData.Id == ownerUserId)
				{
					validateRes.IsCreator = true;
				}
			}

			Globals.ReadCmdArgs().TryGetValue("devconsoleaccess", out string? dcaStr);
			string dca = string.IsNullOrWhiteSpace(dcaStr) ? "private" : dcaStr;
			if (dca == "all" || validateRes.IsCreator)
			{
				consoleAllowed = true;
			}
			else if (dca == "friends")
			{
				Globals.ReadCmdArgs().TryGetValue("ownerfriends", out string? friendsStr);
				if (!string.IsNullOrWhiteSpace(friendsStr))
				{
					foreach (string fid in friendsStr.Split(','))
					{
						if (int.TryParse(fid, out int f) && f == userData.Id) { consoleAllowed = true; break; }
					}
				}
			}
		}
		...
		var (isBanned, banReason, banExpiry) = Root.Bans?.CheckBanned(userData.Id) ?? (false, "", null);
		if (isBanned)
		{
			string banMsg = $"You have been banned from this game.\nReason: {banReason}";
		...
		plr.IsVerified = userData.IsVerified;
		plr.HasTwoFactor = userData.TwoFactorEnabled;
		plr.VoiceChatEnabled = userData.VoiceChatEnabled;
	...
	private void ConnectionFailed(NetworkInstance.NetInstanceErrorEnum netInstanceError)
	{
		...
		if (!string.IsNullOrWhiteSpace(_clientEndpointLabel))
			errMsg += $" ({_clientEndpointLabel})";
	...
	{
		PT.Print($"{Globals.AppDisplayName} Server Started");
		if (IsProd)
		{
	...
	public async void ShutdownServer()
	{
		...
		if (IsProd)
		{
			...
			{
				PT.PrintErr("ServerStopped log failed: ", ex);
			}
		}
	...
	{
		PT.Print($"{Globals.AppDisplayName} Network session started");
	}
	...
	public void OnLocalPlayerReady()
	{
		if (Globals.IsInGDEditor)
		{
			DisplayServer.WindowSetTitle($"{Globals.AppDisplayName} - Client [{LocalPeerID}]");
		}
	...
	/// <summary>Client: send a 20ms Opus voice frame to the server for proximity relay.</summary>
	public void SendVoiceUp(byte[] opus)
	{
		if (IsServer || opus.Length == 0) return;
		RpcId(1, nameof(NetVoiceUp), opus);
	}

	[NetRpc(AuthorityMode.Any, TransferMode = TransferMode.Unreliable, CallLocal = false)]
	private void NetVoiceUp(byte[] opus)
	{
		if (!IsServer || opus.Length == 0) return;

		int senderPeer = RemoteSenderId;
		Player? sender = Root.Players.GetPlayerFromPeerID(senderPeer);
		if (sender == null) return;
		if (!sender.VoiceChatEnabled || !sender.HasTwoFactor) return; // must have voice chat enabled (requires 2FA)

		Vector3 origin = sender.Position;
		int speakerUserId = sender.UserID;

		foreach (Player listener in Root.Players.GetPlayers())
		{
			if (listener.PeerID == senderPeer) continue;
			if (!listener.VoiceChatEnabled) continue; // only deliver to players who have voice chat on
			if (listener.Position.DistanceTo(origin) > VoiceRange) continue;
			RpcId(listener.PeerID, nameof(NetVoiceDown), speakerUserId, opus);
		}
	}

	[NetRpc(AuthorityMode.Server, TransferMode = TransferMode.Unreliable, CallLocal = false)]
	private void NetVoiceDown(int speakerUserId, byte[] opus)
	{
		VoiceReceived?.Invoke(speakerUserId, opus);
	}
	...
	private static string FormatBanDuration(TimeSpan ts)
	{
		if (ts.TotalDays >= 365) return $"{(int)(ts.TotalDays / 365)} year(s)";
		if (ts.TotalDays >= 1)   return $"{(int)ts.TotalDays} day(s)";
		if (ts.TotalHours >= 1)  return $"{(int)ts.TotalHours} hour(s)";
		if (ts.TotalMinutes >= 1) return $"{(int)ts.TotalMinutes} minute(s)";
