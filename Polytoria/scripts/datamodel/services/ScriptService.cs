...
public sealed partial class ScriptService : Instance
{
	...
	public ScriptService()
	{
		if (Globals.UseNodes)
		{
			// Only allow node creation here; scripting is disabled in non-node envs (eg. unit tests).
			// Exported creator builds should still be able to open/create projects if Luau native
			// dependencies are temporarily missing or blocked.
			try
			{
			...
			{
				PT.PrintErr("Failed to initialize Luau provider: ", ex);
			}
		}
	}
