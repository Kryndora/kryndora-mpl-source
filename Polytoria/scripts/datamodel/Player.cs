...
public sealed partial class Player : NPC
{
	...
	/// <summary>Whether this player has two-factor auth enabled. Required to use voice chat.</summary>
	[ScriptProperty, SyncVar]
	public bool HasTwoFactor { get; internal set; }

	/// <summary>Whether this player has enabled voice chat (on the website). Off by default.</summary>
	[ScriptProperty, SyncVar]
	public bool VoiceChatEnabled { get; internal set; }
	...
	private void CopyInventory()
	{
		...
		StarterKit starterKit = Root.StarterKit;
		if (starterKit != null && starterKit.Enabled && (!_hasSpawnedOnce || starterKit.GiveOnRespawn))
		{
			foreach (Instance item in starterKit.GetChildren())
			{
				NetworkedObject a = item.Clone();
				if (a is Instance i)
				{
	...
	public void ResetAppearance()
	{
		...
		if (AutoLoadAppearance)
		{
			...
			{
				int soloAvatarUserId = Root.Entry.LocalTestBaseUserID > 0 ? Root.Entry.LocalTestBaseUserID : 1144;
				LoadAppearance(soloAvatarUserId);
			}
			else
			{
