...
// LocalMeshAsset is in the same namespace
using System;
...
public sealed partial class Mesh : Entity
{
	...
	/// <summary>
	/// Directly supplies a runtime-loaded PackedScene (e.g. from an imported FBX/glTF)
	/// as this Mesh's asset without going through the asset server.
	/// </summary>
	internal void LoadLocalScene(PackedScene scene)
	{
		LocalMeshAsset asset = New<LocalMeshAsset>();
		Asset  = asset;
		asset.SetScene(scene);
	}

	internal void LoadLocalFile(FileLinkAsset sourceFile)
	{
		LocalMeshAsset asset = New<LocalMeshAsset>();
		Asset = asset;
	...
	internal void LoadLocalFile(FileLinkAsset sourceFile, PackedScene previewScene)
	{
		LocalMeshAsset asset = New<LocalMeshAsset>();
		Asset = asset;
		asset.SourceFile = sourceFile;
		asset.SetScene(previewScene);
	}

	internal void LoadLocalFile(FileLinkAsset sourceFile, PackedScene previewScene, string importedNodePath, int importedSurfaceIndex)
	{
		LocalMeshAsset asset = New<LocalMeshAsset>();
		Asset = asset;
		asset.ImportedNodePath = importedNodePath;
		asset.ImportedSurfaceIndex = importedSurfaceIndex;
		asset.SourceFile = sourceFile;
		asset.SetScene(previewScene);
	}
	...
	private void OnResourceLoaded(Resource resource)
	{
		...
		if (resource is PackedScene scene)
		{
			...
			_animPlay = FindAnimationPlayer(_meshNode);
				...
				if (item is MeshInstance3D m3d)
				{
					...
					for (int i = 0; i < surfaceCount; i++)
					{
						...
						Material mat = sourceMaterial != null
							? (Material)sourceMaterial.Duplicate()
	...
	private static AnimationPlayer? FindAnimationPlayer(Node node)
	{
		foreach (Node child in node.GetChildren())
		{
			...
			AnimationPlayer? nested = FindAnimationPlayer(child);
			if (nested != null)
			{
				return nested;
			}
