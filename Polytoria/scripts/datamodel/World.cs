...
public sealed partial class World : Instance
{
	...
	public static World? Current
	{
		...
		set
		{
			...
			{
				DisplayServer.WindowSetTitle($"{value.LinkedSession.Metadata.ProjectName} - {Globals.CreatorDisplayName} v{Globals.AppVersion}");
	...
	private async void FetchWorldInfo()
	{
		...
		if (WorldInfo.HasValue)
		{
			...
			DisplayServer.WindowSetTitle($"{TitleEllipsis(WorldInfo.Value.Name, 50)} - {Globals.AppDisplayName} v{Globals.AppVersion}");
	...
	public World Setup()
	{
		...
		StarterKit? starterKit = FindChild<StarterKit>("StarterKit");

		if (starterKit == null)
		{
			starterKit = Globals.LoadInstance<StarterKit>(Root);
			starterKit.NetworkParent = this;
		}
		...
		FilterServiceKryndora? filterService = FindChild<FilterServiceKryndora>("Filter");

		if (filterService == null)
		{
			filterService = Globals.LoadInstance<FilterServiceKryndora>(Root);
		...
		PreferencesServiceKryndora? preferencesService = FindChild<PreferencesServiceKryndora>("Preferences");
		if (preferencesService == null)
		{
			preferencesService = Globals.LoadInstance<PreferencesServiceKryndora>(Root);
		...
		BanService? banService = FindChild<BanService>("BanService");
		if (banService == null)
		{
			banService = Globals.LoadInstance<BanService>(Root);
			banService.NameOverride = "BanService";
