	...
	public struct LegacyParser : IXmlReadHandler
	{
		...
		public readonly GameItem Root
		{
			get
			{
				...
				if (_items.TryPeek(out GameItem? item))
				{
					return item;
				}

				throw new InvalidDataException("No model data was found in this file.");
			}
		}
...
#pragma warning restore IL2074
		...
		public void OnEndTag(ReadOnlySpan<char> name, int line, int column)
		{
			...
			{
				if (_items.TryPop(out GameItem? item) && item.Class != null)
				{
					if (_items.TryPeek(out GameItem? parent))
					{
...
#pragma warning restore IL2026 // Members annotated with 'RequiresUnreferencedCodeAttribute' require dynamic access otherwise can break functionality when trimming application code
	...
	{
		if (IsStaticModelRoot(item))
		{
			return SpawnStaticModelRootAsModel(item, parent, game);
		}

		return SpawnItem(item, parent, game);
	}

	private static bool IsStaticModelRoot(GameItem item)
	{
		return item.Class != null && item.Class.IsDefined(typeof(StaticAttribute), true);
	}

	private static Model? SpawnStaticModelRootAsModel(GameItem item, Instance parent, World root)
	{
		Model? model = Globals.LoadInstance<Model>(root);
		if (model == null)
		{
			return null;
		}

		model.Name = string.IsNullOrWhiteSpace(item.Name) || item.Name == "World" || IsFixedServiceName(item.Class)
			? "Imported Model"
			: item.Name;
		model.Root = root;
		model.AutoInvokeReady = false;
		model.CallInitOverrides = false;
#if CREATOR
		model.CreatorInserted();
#endif
		model.SetNetworkParent(parent, force: true);

		foreach (GameItem child in item.Children)
		{
			SpawnModelImportChild(child, model, root);
		}

		model.InvokePropReady();
		return model;
	}

	private static void SpawnModelImportChild(GameItem item, Instance parent, World root)
	{
		if (IsStaticModelRoot(item))
		{
			foreach (GameItem child in item.Children)
			{
				SpawnModelImportChild(child, parent, root);
			}

			return;
		}

		SpawnItem(item, parent, root);
	}

	private static bool IsFixedServiceName(Type? type)
	{
		return type != null && _fixedServiceNames.ContainsKey(type);
	}
