...
public static partial class PackedFormat
{
	...
	private static async Task PackModelToArchive(Instance model, ZipArchive archive, IProgress<LoadOverlayProgress>? progress = null)
	{
		...
		{
			i++;
			bool hadUsed = IsFileLinkUsedByModel(f, model);
			if (!hadUsed) continue;
	...
	private static bool IsFileLinkUsedByModel(FileLinkAsset fileLink, Instance model)
	{
		foreach (NetworkedObject item in fileLink.LinkedTo)
		{
			if (IsObjectInsideModel(item, model))
			{
				return true;
			}

			if (item is BaseAsset asset)
			{
				foreach (NetworkedObject linkedObject in asset.LinkedTo)
				{
					if (IsObjectInsideModel(linkedObject, model))
					{
						return true;
					}
				}
	...
	private static bool IsObjectInsideModel(NetworkedObject item, Instance model)
	{
		return item == model || (item is Instance instance && instance.IsDescendantOf(model));
	}
